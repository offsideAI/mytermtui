...
func (e *Editor) updateNormal(m *Model, msg tea.KeyMsg) tea.Cmd {
	...
	case "u":
		e.doUndo()
	case "U": // redo — ctrl+r is taken by "run query" in the SQL tab
		e.doRedo()
...
func (e *Editor) runExCommand(m *Model) tea.Cmd {
	...
	if write {
		...
		return editorSubmit(m, e)
	}
	if quit {
		// The SQL buffer lives on in its session — :q only parks focus,
		// so a "modified" buffer is never lost and needs no ! (unlike
		// the file-editor original this was ported from).
		_ = force
		e.quit = true
	}
	return nil
...
func (e *Editor) View(m *Model, width, height int) []string {
	...
	out[0] = pad(t.PanelTitle.Render(title), width)

	// Body.
	for i := 0; i < rows; i++ {
		ln := e.top + i
		if ln >= len(e.lines) {
			out[1+i] = pad(t.Dim.Render(" ~"), width)
			continue
		}
		num := t.Dim.Render(fmt.Sprintf("%*d ", gutter-1, ln+1))
...
func (e *Editor) renderCursorLine(t Theme, text string, textW, width, gutter int) string {
	...
	num := t.Dim.Render(fmt.Sprintf("%*d ", gutter-1, e.row+1))
...
func (e *Editor) statusLine(t Theme, width int) string {
	...
	default:
		if e.status != "" {
			left = t.Dim.Render(e.status)
		} else if e.pending != "" {
			left = t.Dim.Render("(" + e.pending + ")")
		}
	}
