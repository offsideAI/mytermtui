...
func (m *Model) resetLivePane() {
	m.entries = nil
	// Fresh slice, not a truncation: [:0] would alias the parked
...
// resizePaneAction adjusts the left panel's share of the width.
func (m *Model) resizePaneAction(delta float64) tea.Cmd {
	if !m.split() {
		return m.note(levelInfo, "no split to resize")
	}
	m.ratio += delta
	if m.ratio < 0.15 {
		m.ratio = 0.15
...
func (m *Model) liveRef() paneRef {
	return paneRef{
		...
		anchor: m.anchor, filter: m.filterText, live: true,
	}
}
