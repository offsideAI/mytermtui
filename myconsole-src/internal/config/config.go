// Package config loads myconsole's optional TOML configuration and
...
type General struct {
	...
	// EnterOpensFile controls what enter does on a file: "editor" opens
	// it in the embedded vim-style editor (myconsole default); "reveal"
	// shows it in Finder; "app" opens it in its default application (the
	// o key always opens in the app regardless).
...
// DefaultPath is ~/.config/myconsole/config.toml (per spec; not
...
func DefaultPath() string {
	...
	return filepath.Join(home, ".config", "myconsole", "config.toml")
}
...
func StateDir() string {
	...
	return filepath.Join(home, ".local", "state", "myconsole")
}
...
func Load(path string) (Config, error) {
	...
	switch cfg.General.EnterOpensFile {
	case "app", "reveal", "editor":
	default:
		cfg.General.EnterOpensFile = "editor"
	}
