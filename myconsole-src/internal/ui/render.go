...
func (m *Model) renderMain() []string {
	...
	showRight := (m.previewOn || m.editor != nil) && m.w >= 70
	if showRight {
		...
		switch {
		case m.editor != nil:
			left = m.paneLines(m.liveRef(), lw, !m.editorFocused, listH)
			right = m.editor.View(m, rw, listH+2)
		case m.focusRight:
			left = m.paneLines(refOf(m.other), lw, false, listH)
			right = m.paneLines(m.liveRef(), rw, true, listH)
		default:
			left = m.paneLines(m.liveRef(), lw, true, listH)
			right = m.renderContentsSide(rw, listH)
		}
		...
		lines = m.paneLines(m.liveRef(), m.w, true, listH)
	}
...
// renderContentsSide draws the passive right panel while the tree is
// focused: the selected folder's listing, a file's metadata + preview,
// or a placeholder while the listing loads.
func (m *Model) renderContentsSide(width, listH int) []string {
	e := m.currentEntry()
	if e != nil && !e.IsDir {
		return m.previewBlock(*e, width, listH+2)
	}
	if e != nil && e.IsDir && m.other != nil && m.other.cwd == e.Path {
		return m.paneLines(refOf(m.other), width, false, listH)
	}
	out := make([]string, listH+2)
	for i := range out {
		line := ""
		if i == 2 && e != nil {
			line = " " + m.theme.HiddenDim.Render("(loading…)")
		}
...
// previewBlock renders file metadata plus the guarded text preview as
// the right panel's content.
func (m *Model) previewBlock(e fsx.Entry, width, height int) []string {
	t := m.theme
	inner := width - 2
	if inner < 4 {
		inner = 4
	...
	content := []string{
		" " + t.PreviewTitle.Render(truncMiddle(sanitize(e.Name), inner-1)),
		" " + t.PreviewMeta.Render(truncEnd(e.Kind()+" · "+humanBytes(e.Size), inner-1)),
		"",
	}
	for _, l := range m.prevLines {
		content = append(content, " "+truncEnd(l, inner-1))
	}
...
func (m *Model) renderRowRef(ref paneRef, vi, width int, focused bool) string {
	...
	icon = indent + icon
	nameW := nameColWidth(width) - 2*e.Depth
	if nameW < 4 {
		nameW = 4
	}
