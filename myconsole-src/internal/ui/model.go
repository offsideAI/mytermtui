...
type Model struct {
	...
	cwd     string
	home    string
	entries []fsx.Entry // flattened tree in display order
	view    []int       // indexes into entries (identity after flatten)
	// Tree expansion: rootEntries is the raw cwd listing; expanded dirs
	// pull their cached child listings into the flattened entries.
	...
	editor        *Editor // embedded editor (nil when not editing)
	editorFocused bool    // whether keys route to the editor
...
func New(cfg config.Config, startDir string, bridge icloud.Bridge, queue *icloud.Queue) *Model {
	...
	m := &Model{
		...
		previewOn:   cfg.General.ShowPreview,
...
func loadContentsCmd(path string, focus bool, cursorOn string) tea.Cmd {
	return func() tea.Msg {
		entries, err := fsx.ReadDir(path)
		return contentsLoadedMsg{path: path, entries: entries, err: err, focus: focus, cursorOn: cursorOn}
	}
}

func loadChildrenCmd(parent string) tea.Cmd {
	return func() tea.Msg {
		entries, err := fsx.ReadDir(parent)
		return childrenLoadedMsg{parent: parent, entries: entries, err: err}
	}
}
...
func (m *Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case dirLoadedMsg:
		...
		cmds := []tea.Cmd{m.followCursor()}
		if !changedDir {
			// Refresh the listings of expanded folders so the tree does
			// not go stale after ops or download completions (bounded).
			n := 0
			for dir := range m.expanded {
				if n >= 50 {
					break
				}
				cmds = append(cmds, loadChildrenCmd(dir))
				n++
			}
		}
	...
	case contentsLoadedMsg:
		if msg.err != nil {
			if msg.focus {
				return m, m.note(levelErr, errHint(msg.err))
			}
			return m, nil
		}
		p := m.makeContentsPane(msg.path, msg.entries, msg.cursorOn)
		if msg.focus {
			if m.focusRight { // rapid re-focus: just replace the live pane
				m.restorePane(p)
				return m, nil
			}
			cur := m.snapshotPane()
		...
		if m.focusRight {
			return m, nil // stale follow while the panel is focused
		}
		if cur := m.currentEntry(); cur == nil || cur.Path != msg.path {
			return m, nil // cursor moved on before the listing arrived
		}
		m.other = p
		return m, nil
	...
	case childrenLoadedMsg:
		...
		m.childCache[msg.parent] = msg.entries
		if m.expanded[msg.parent] {
			m.rebuildView("")
		}
		return m, m.followCursor()
	...
	case editorOpenMsg:
		if msg.err != nil {
			return m, m.note(levelErr, "open: "+errHint(msg.err))
		}
		if msg.tooBig {
			return m, m.note(levelWarn, "file too large to edit (>10 MB)")
		}
		if msg.binary {
			return m, m.note(levelWarn, "binary file — not editable")
		}
		m.editor = newEditor(msg.path, msg.data, false)
	...
	case editorSavedMsg:
		if m.editor != nil && m.editor.path == msg.path {
			if msg.err != nil {
				m.editor.status = "write failed: " + msg.err.Error()
			} else {
				m.editor.modified = false
				m.editor.status = "\"" + filepath.Base(msg.path) + "\" written"
			}
		}
		return m, nil
...
func (m *Model) updateKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	// The editor grabs all keys while focused. tab in normal mode parks
	// focus back on the tree without closing the editor.
	if m.editor != nil && m.editorFocused {
		if key == "tab" && m.editor.mode == edNormal {
			m.editorFocused = false
			return m, m.followCursor()
		}
		cmd := m.editor.Update(m, msg)
		if m.editor.quit {
			m.editor = nil
			m.editorFocused = false
			return m, m.followCursor()
	...
	if act, ok := m.keys.Lookup(key); ok {
		// Explorer semantics for ← (and h); backspace stays a plain
		// go-to-parent in whichever pane is focused.
		if act == ActParent && key != "backspace" {
			if m.previewOn && m.focusRight {
				return m, m.dispatch(ActSwapPane) // back to the tree
			}
			return m, m.smartLeftAction()
		}
...
// flatten materializes the visible tree: the sorted, filtered root
// listing with each expanded folder's (sorted, filtered) children
// inlined beneath it at increasing depth. Fresh slices every time —
// truncating shared arrays corrupts parked panels.
func (m *Model) flatten() {
	needle := strings.ToLower(m.filterText)
	var out []fsx.Entry
	var walk func(list []fsx.Entry, depth int)
	walk = func(list []fsx.Entry, depth int) {
		sorted := append([]fsx.Entry(nil), list...)
		fsx.Sort(sorted, m.sortBy, m.sortAsc, m.dirsFirst)
		for _, e := range sorted {
			...
			if e.IsDir && m.expanded[e.Path] {
				if kids, ok := m.childCache[e.Path]; ok {
					walk(kids, depth+1)
				}
			}
		}
	}
	walk(m.rootEntries, 0)
	m.entries = out
