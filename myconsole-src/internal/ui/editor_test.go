...
func TestEditorExWriteQuit(t *testing.T) {
	e := ed(t, "hi")
	feed(t, e, "x") // modify (delete 'h')
	// :q on a modified buffer is refused
	feed(t, e, ":q<cr>")
	if e.quit {
		t.Fatal(":q quit a modified buffer")
	}
	if !strings.Contains(e.status, "no write") {
		t.Fatalf("expected no-write warning, got %q", e.status)
	}
	// :q! forces
	feed(t, e, ":q!<cr>")
	if !e.quit {
...
func TestEditorWriteCommandReturnsSaveCmd(t *testing.T) {
	e := ed(t, "data")
	feed(t, e, "x") // modify
	cmd := e.Update(nil, tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{':'}})
	if cmd != nil {
		t.Fatal("entering command mode should not save")
	}
	feed(t, e, "w")
	cmd = e.Update(nil, tea.KeyMsg{Type: tea.KeyEnter})
	if cmd == nil {
		t.Fatal(":w did not return a save command")
	}
}
