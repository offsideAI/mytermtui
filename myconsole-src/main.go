// myconsole: a keyboard-driven filesystem browser and iCloud file syncer.
package main
...
func main() {
	...
	flag.Usage = func() {
		fmt.Fprintf(os.Stderr, "usage: myconsole [flags] [start-dir]\n\n")
	...
	if *showVersion {
		fmt.Println("myconsole", version)
		return
	}

	cfg, err := config.Load(*cfgPath)
	if err != nil {
		fmt.Fprintf(os.Stderr, "myconsole: config %s: %v (using defaults)\n", *cfgPath, err)
	}
	...
	if fi, err := os.Stat(startDir); err != nil || !fi.IsDir() {
		fmt.Fprintf(os.Stderr, "myconsole: %s is not a browsable directory\n", startDir)
		os.Exit(1)
	}
	...
	if _, err := p.Run(); err != nil {
		fmt.Fprintf(os.Stderr, "myconsole: %v\n", err)
		os.Exit(1)
	}
}
